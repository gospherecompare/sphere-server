...
const API_ALIAS_REWRITE_RULES = [
  ...
  { alias: /^\/api\/gateway\/catalog\/network-grid$/i, target: "/api/networking" },
  ...
  { alias: /^\/api\/gateway\/channel\/stores$/i, target: "/api/public/online-stores" },
  { alias: /^\/api\/gateway\/insight\/features$/i, target: "/api/public/popular-features" },
  { alias: /^\/api\/gateway\/pulse\/handset$/i, target: "/api/public/trending/smartphones" },
  { alias: /^\/api\/gateway\/pulse\/compute$/i, target: "/api/public/trending/laptops" },
  { alias: /^\/api\/gateway\/pulse\/vision$/i, target: "/api/public/trending/tvs" },
  { alias: /^\/api\/gateway\/pulse\/network-grid$/i, target: "/api/public/trending/networking" },
  { alias: /^\/api\/gateway\/pulse\/duel$/i, target: "/api/public/trending/most-compared" },
  { alias: /^\/api\/gateway\/pulse\/all$/i, target: "/api/public/trending/all" },
  { alias: /^\/api\/gateway\/release\/handset$/i, target: "/api/public/new/smartphones" },
  { alias: /^\/api\/gateway\/release\/compute$/i, target: "/api/public/new/laptops" },
  { alias: /^\/api\/gateway\/release\/vision$/i, target: "/api/public/new/tvs" },
  { alias: /^\/api\/gateway\/release\/network-grid$/i, target: "/api/public/new/networking" },
  { alias: /^\/api\/gateway\/event\/feature$/i, target: "/api/public/feature-click" },
  { alias: /^\/api\/gateway\/compare\/log$/i, target: "/api/public/compare" },
  { alias: /^\/api\/gateway\/compare\/score$/i, target: "/api/public/compare/scores" },
  { alias: /^\/api\/gateway\/compare\/resolve$/i, target: "/api/public/compare/resolve" },
...
const resolveUniqueBlogSlug = async (requestedSlug, productId, blogId = null) => {
  const hasProductId = Number.isInteger(Number(productId)) && Number(productId) > 0;
  ...
  while (true) {
    ...
    if (Number.isInteger(Number(blogId)) && Number(blogId) > 0 && existingId === Number(blogId)) {
      return slug;
    }
...
app.post("/api/admin/blogs", authenticate, async (req, res) => {
  try {
    ...
    if (productId) {
      const profileConfig = await readDeviceFieldProfilesConfig();
      snapshot = await fetchBlogProductSnapshot(productId, profileConfig.profiles);
      if (!snapshot) {
        return res.status(404).json({ message: "Product not found" });
      }
    }
